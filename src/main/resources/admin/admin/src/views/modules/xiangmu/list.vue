<template>
    <div class="main-content">
        <el-dialog title="审核" :visible.sync="xiangmuYesnoTypesVisible">
            <el-form :model="form">
                <input type="hidden" v-model="form.id">
                <el-form-item label="审核" >
                    <el-select v-model="form.xiangmuYesnoTypes" placeholder="请选择申报状态">
                        <el-option label="同意" value="2"></el-option>
                        <el-option label="拒绝" value="3"></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label="申报结果">
                    <el-input type="textarea" v-model="form.xiangmuYesnoText" placeholder="申报结果"></el-input>
                </el-form-item>
            </el-form>
            <div slot="footer" class="dialog-footer">
                <el-button @click="xiangmuYesnoTypesVisible = false">取 消</el-button>
                <el-button type="primary" @click="xiangmuYesnoTypesShenhe">审 核</el-button>
            </div>
        </el-dialog>
        <!-- 条件查询 -->
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '申报状态' : ''">
                        <el-select v-model="searchForm.xiangmuYesnoTypes" placeholder="请选择申报状态">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                    ...
                    <el-form-item :label="contents.inputTitle == 1 ? '教师名称' : ''">
                        <el-input prefix-icon="el-icon-search" v-model="searchForm.jiaoshiName" placeholder="教师名称" clearable></el-input>
                    </el-form-item>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('xiangmu','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/daxueshengcycxin/upload/xiangmuMuBan.xls"
                        >批量导入科研项目申请数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('xiangmu','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="xiangmuUploadSuccess"
                                :on-error="xiangmuUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('xiangmu','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入科研项目申请数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('xiangmu','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "xiangmu.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="xiangmuFile"
                        ...
                        <template slot-scope="scope">
                            <div v-if="scope.row.xiangmuFile">
                                <a type="text" style="text-decoration:none" class="el-button" :href="$base.url+scope.row.xiangmuFile">下载</a>
                            </div>
                            <div v-else>无文件</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="xiangmuChuangxin"
                                   header-align="center"
                                   label="项目创新点">
                        <template slot-scope="scope">
                            {{scope.row.xiangmuChuangxin}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="xiangmuText"
                                      ...
                                      label="项目实施方案">
                        <template slot-scope="scope">
                            <span v-if="scope.row.xiangmuText != null &&scope.row.xiangmuText.length>10">
                                {{scope.row.xiangmuText.slice(0,10)}}...
                            </span>
                            <span v-else>
                                {{scope.row.xiangmuText}}
                            </span>
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="xiangmuYesnoText"
                                      header-align="center"
                                      label="申报结果">
                        <template slot-scope="scope">
                            <span v-if="scope.row.xiangmuYesnoText != null &&scope.row.xiangmuYesnoText.length>10">
                                {{scope.row.xiangmuYesnoText.slice(0,10)}}...
                            </span>
                            <span v-else>
                                {{scope.row.xiangmuYesnoText}}
                            </span>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('xiangmu','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('xiangmu','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('xiangmu','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
                            <el-button v-if="isAuth('xiangmu','审核') && scope.row.xiangmuYesnoTypes == 1 " type="primary" icon="el-icon-thumb" size="mini" @click="openYesnoTypes(scope.row.id)">审核</el-button>
                            <el-button v-if="isAuth('xiangmu','申请') && scope.row.xiangmuYesnoTypes == 2 " type="primary" icon="el-icon-thumb" size="mini" @click="shenqing(scope.row.id)">申请项目</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
            ...
            form:{
                ...
                xiangmuFile : null,
                jiaoshiId : null,
                xiangmuChuangxin : null,
                xiangmuText : null,
                xiangmuContent : null,
                xiangmuYesnoTypes : null,
                xiangmuYesnoText : null,
                xiangmuDelete : null,
                insertTime : null,
                createTime : null,
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'xiangmu',//当前表表名,
                        sumColum : 'xiangmu_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'xiangmu_name',//分组字符串字段
                        // types : 'xiangmu_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
			...
			shenqing(id){
				...
				this.$confirm(`确定要申请这个项目吗?`, "提示", {
				    confirmButtonText: "确定",
				    cancelButtonText: "取消",
				    type: "warning"
				}).then(() => {
				    this.$http({
				        url: "xuantishenhe/save",
				        method: "post",
				        data: data
				    }).then(({data}) => {
				        if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            xiangmuUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "xiangmu/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
                            message: "导入科研项目申请数据成功",
                            type: "success",
                            duration: 1500,
                            onClose: () => {
                                _this.search();
            ...
            openYesnoTypes(id) {
                ...
                _this.form.id = null;
                _this.form.id = id;
                            _this.form.xiangmuYesnoText = null;
                _this.form.xiangmuYesnoTypes = "请选择申报状态";
                _this.xiangmuYesnoTypesVisible = true;
            },

            xiangmuYesnoTypesShenhe() {
                let _this = this;
                if(_this.form.xiangmuYesnoTypes == "请选择申报状态"){
                    _this.$message.error("请选择申报状态");return false;
                }
                if(_this.form.xiangmuYesnoText == null || _this.form.xiangmuYesnoText == ""){
                    _this.$message.error("请输入申报结果");return false;
                }
                this.$http({
                    url:`xiangmu/shenhe`,
                    method: "post",
