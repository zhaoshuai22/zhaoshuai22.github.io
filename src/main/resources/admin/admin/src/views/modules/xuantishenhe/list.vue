<template>
    <div class="main-content">
        <el-dialog title="审核" :visible.sync="xuantishenheYesnoTypesVisible">
            <el-form :model="form">
                <input type="hidden" v-model="form.id">
                <el-form-item label="审核" >
                    <el-select v-model="form.xuantishenheYesnoTypes" placeholder="请选择选题申请">
                        <el-option label="同意" value="2"></el-option>
                        <el-option label="拒绝" value="3"></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label="申请结果">
                    <el-input type="textarea" v-model="form.xuantishenheYesnoText" placeholder="申请结果"></el-input>
                </el-form-item>
            </el-form>
            <div slot="footer" class="dialog-footer">
                <el-button @click="xuantishenheYesnoTypesVisible = false">取 消</el-button>
                <el-button type="primary" @click="xuantishenheYesnoTypesShenhe">审 核</el-button>
            </div>
        </el-dialog>
        <!-- 条件查询 -->
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '选题申请' : ''">
                        <el-select v-model="searchForm.xuantishenheYesnoTypes" placeholder="请选择选题申请">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in xuantishenheYesnoTypesSelectSearch"
                               v-bind:key="index"
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('xuantishenhe','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/daxueshengcycxin/upload/xuantishenheMuBan.xls"
                        >批量导入课题审核数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('xuantishenhe','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="xuantishenheUploadSuccess"
                                :on-error="xuantishenheUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('xuantishenhe','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入课题审核数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('xuantishenhe','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "xuantishenhe.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="xuantishenheYesnoTypes"
                                      header-align="center"
                                      label="选题申请">
                        <template slot-scope="scope">
                            {{scope.row.xuantishenheYesnoValue}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="xuantishenheYesnoText"
                                      header-align="center"
                                      label="申请结果">
                        <template slot-scope="scope">
                            <span v-if="scope.row.xuantishenheYesnoText != null &&scope.row.xuantishenheYesnoText.length>10">
                                {{scope.row.xuantishenheYesnoText.slice(0,10)}}...
                            </span>
                            <span v-else>
                                {{scope.row.xuantishenheYesnoText}}
                            </span>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('xuantishenhe','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('xuantishenhe','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('xuantishenhe','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
                            <el-button v-if="isAuth('xuantishenhe','审核') && scope.row.xuantishenheYesnoTypes == 1 " type="primary" icon="el-icon-thumb" size="mini" @click="openYesnoTypes(scope.row.id)">审核</el-button>
                            <el-button v-if="isAuth('xuantishenhe','项目') && scope.row.xuantishenheYesnoTypes != 1 && scope.row.xuantishenheYesnoTypes != 3 && scope.row.xuantishenheYesnoTypes != 6" type="primary" icon="el-icon-edit" size="mini" @click="xiangmjindu(scope.row)">上传项目进度</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
            ...
            form:{
                ...
                xiangmuId : null,
                yonghuId : null,
                xuantishenheYesnoTypes : null,
                xuantishenheYesnoText : null,
                xuantishenheDelete : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
            ...
            xuantishenheYesnoTypesVisible : false,
            ...
            json_fields: {
                //级联表字段
                     ...
                     '项目经费': 'xiangmuJingfei',
                     '项目文件': 'xiangmuFile',
                     '项目创新点': 'xiangmuChuangxin',
                     '项目实施方案': 'xiangmuText',
                     '申报状态': 'xiangmuYesnoTypes',
                     '申报结果': 'xiangmuYesnoText',
                     '学号': 'yonghuUuidNumber',
                     '学生名称': 'yonghuName',
                     '学生手机号': 'yonghuPhone',
                     '学生头像': 'yonghuPhoto',
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'xuantishenhe',//当前表表名,
                        sumColum : 'xuantishenhe_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'xuantishenhe_name',//分组字符串字段
                        // types : 'xuantishenhe_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                //         groupColumn: "xuantishenhe_types",
                //// riqi :getMonthFormat(_this.echartsDate),//年月
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                params['xuantishenheDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "xuantishenhe/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            xuantishenheUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "xuantishenhe/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            xuantishenheUploadError(data){
                ...
                _this.form.id = null;
                _this.form.id = id;
                            _this.form.xuantishenheYesnoText = null;
                _this.form.xuantishenheYesnoTypes = "请选择选题申请";
                _this.xuantishenheYesnoTypesVisible = true;
            },

            xuantishenheYesnoTypesShenhe() {
                let _this = this;
                if(_this.form.xuantishenheYesnoTypes == "请选择选题申请"){
                    _this.$message.error("请选择选题申请");return false;
                }
                if(_this.form.xuantishenheYesnoText == null || _this.form.xuantishenheYesnoText == ""){
                    _this.$message.error("请输入申请结果");return false;
                }
                this.$http({
                    url:`xuantishenhe/shenhe`,
                    method: "post",
                    data: _this.form
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        this.$message({
                            ...
                            duration: 1500,
                            onClose: () => {
                                _this.xuantishenheYesnoTypesVisible = false;
                                _this.search();
                            }
                        });
                    } else {
